  ...
  const handlePasskeyAction = async (isRegistration: boolean) => {
    ...
    try {
      if (isRegistration) {
        ...
        // 認証結果をサーバーに送信
        const verificationResponse = await fetch('/api/passkey/login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <Card className="w-[350px] mx-auto">
      <CardHeader>
        ...
        <CardDescription>メールアドレスを入力してパスキーで認証してください</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          >
            ...
            {isRegistering ? 'パスキーを登録' : 'パスキーでログイン'}
          </Button>
          <Button
            variant="outline"
            className="w-full"
          ...
          >
            {isRegistering ? 'ログインに戻る' : 'パスキーを登録する'}
          </Button>
